...
export default function Home() {
  ...
  const handleScanSuccess = (
    decodedText: string,
    detectedFormat: 'qr' | 'code128' | 'ean13' | 'ean8' | 'upca' | 'code39',
  ) => {
    ...
    const payload = {
      ...
      accountNumber:
        editCategory === 'duitnow'
          ? editAccountNumber.trim() || undefined
          : undefined,
    };
  ...
  return (
    <div className='min-h-screen bg-black text-zinc-100 font-sans pb-24'>
      ...
      <div className='absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-lg h-[400px] bg-emerald-950/20 rounded-full blur-3xl pointer-events-none' />
      ...
      <div className='max-w-md mx-auto px-4 pt-8'>
        ...
        <header className='flex justify-between items-center mb-8'>
          ...
          <Link
            href='/add'
            className='flex items-center justify-center w-11 h-11 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-[0_4px_12px_rgba(16,185,129,0.25)] transition-all duration-200 active:scale-95 border border-emerald-500/20'
            aria-label='Add Card'>
        ...
        <div className='relative mb-6'>
          ...
          <input
            ...
            className='w-full pl-4 pr-4 py-3.5 bg-zinc-900/60 backdrop-blur-md rounded-2xl border border-zinc-800 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/30 transition-all shadow-inner'
          />
        </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all duration-200 ${
                  selectedCategory === cat.id
                    ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/10'
                    : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800 hover:text-zinc-205'
                }`}>
                {cat.label}
              </button>
            ))}
        ...
        {loading ? (
          ...
          <div className='flex flex-col items-center justify-center text-center p-8 bg-zinc-900/30 border border-dashed border-zinc-800 rounded-3xl mt-4'>
            ...
            <h3 className='font-semibold text-zinc-300 text-base mb-1'>
            ...
            {!searchQuery && selectedCategory === 'all' && (
              <Link
                href='/add'
                className='inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-semibold transition-all shadow-[0_4px_12px_rgba(16,185,129,0.2)] active:scale-95'>
        ...
        <div className='mt-12 p-4 rounded-2xl bg-zinc-900/40 border border-zinc-800 flex gap-3 items-start'>
          ...
          <div>
            ...
            <p className='text-[10px] text-zinc-500 mt-1 leading-relaxed'>
              Add OrbitWallet to your device's home screen. Open your
              memberships instantly in stores without network.
            </p>
          </div>
        </div>
      </div>
      ...
      {activeCard && (
        <div className='fixed inset-0 z-40 bg-black/80 backdrop-blur-md flex items-center justify-center p-4'>
          <div
            className={`w-full max-w-sm rounded-3xl border shadow-2xl transition-all duration-300 p-6 flex flex-col gap-6 relative max-h-[90vh] overflow-y-auto no-scrollbar ${
              brightMode
                ...
                {!isEditing && (
                  <>
                    <button
                      ...
                      className={`p-2 rounded-xl border transition-all duration-200 active:scale-90 ${
                        brightMode
                          ...
                          : 'bg-zinc-950 border-zinc-800 text-amber-400 hover:text-amber-300'
                      }`}
                    ...
                    <button
                      ...
                      className={`p-2 rounded-xl border transition-colors active:scale-90 ${
                        brightMode
                          ? 'bg-red-50 border-red-100 text-red-600 hover:text-red-700'
                          : 'bg-zinc-950 border-zinc-800 text-red-400 hover:text-red-300'
                      }`}
                      title='Delete card'>
            ...
            {isEditing ? (
              ...
              <form
                ...
                <div className='flex flex-col gap-1'>
                  ...
                  <input
                    ...
                    className='w-full px-3 py-2.5 bg-zinc-955 text-white border border-zinc-800 rounded-xl text-xs focus:outline-none focus:border-emerald-500'
                  />
                </div>
                ...
                <div className='flex flex-col gap-1'>
                  ...
                  <div className='flex gap-2'>
                    <input
                      ...
                      className='flex-1 min-w-0 px-3 py-2.5 bg-zinc-955 text-white border border-zinc-800 rounded-xl text-xs font-mono'
                    />
                    <button
                      type='button'
                      onClick={() => setShowScanner(true)}
                      className='px-2.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl flex items-center justify-center gap-1 transition-colors active:scale-95 text-[10px] font-semibold shrink-0'>
                      <Camera className='w-3.5 h-3.5' />
                    </button>
                    <label className='px-2.5 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-xl flex items-center justify-center gap-1 transition-colors active:scale-95 text-[10px] font-semibold shrink-0 cursor-pointer border border-zinc-700'>
                  ...
                  {isScanningGallery && (
                    <p className='text-[9px] text-emerald-400 mt-0.5 animate-pulse'>
                      Scanning image...
                    </p>
                  )}
                  {galleryError && (
                ...
                <div className='flex flex-col gap-1'>
                  ...
                  <div className='grid grid-cols-3 gap-1.5'>
                    {COLOR_PRESETS.map((preset) => (
                      <button
                        ...
                        className={`h-8 rounded-lg bg-gradient-to-br ${preset.value} flex items-center justify-center transition-all ${
                          editColor === preset.name
                            ? 'ring-2 ring-emerald-400 scale-95 border-none'
                    ...
                    <button
                      type='button'
                      onClick={() =>
                        setEditColor(`${customStart},${customEnd}`)
                      }
                      className={`h-8 rounded-lg flex items-center justify-center transition-all ${
                        editColor.includes(',')
                          ? 'ring-2 ring-emerald-400 scale-95 border-none'
                          : 'border border-zinc-800 bg-zinc-955 opacity-80'
                      }`}
                      style={{
                    ...
                    <div className='flex gap-2 mt-1.5 p-2 bg-zinc-950 border border-zinc-850 rounded-lg'>
                      <div className='flex-1 flex flex-col gap-0.5'>
                        <input
                          ...
                          value={customStart}
                          onChange={(e) =>
                            handleCustomColorChange(e.target.value, customEnd)
                          }
                      ...
                      <div className='flex-1 flex flex-col gap-0.5'>
                        <input
                          ...
                          value={customEnd}
                          onChange={(e) =>
                            handleCustomColorChange(customStart, e.target.value)
                          }
                          className='w-full h-6 border-0 bg-transparent cursor-pointer rounded'
                        />
                      </div>
                ...
                <div className='flex gap-2 mt-2'>
                  ...
                  <button
                    type='submit'
                    className='flex-1 py-3 bg-emerald-600 text-white rounded-xl text-xs font-semibold shadow-md shadow-emerald-600/10'>
                    Save
                  </button>
                </div>
              </form>
              ...
              <div className='flex flex-col items-center gap-6 animate-in fade-in duration-200'>
                ...
                {/* Generated Scan Code */}
                <div
                  className={`w-full p-4 rounded-2xl border shadow-inner flex flex-col items-center gap-3 ${
                    brightMode
                  ...
                  <div className='text-center mt-1'>
                    <p className='font-mono text-base font-bold tracking-widest select-all'>
                      {activeCard.category === 'duitnow'
                        ? activeCard.accountNumber || activeCard.cardNumber
                        : activeCard.cardNumber}
                    </p>
                    <p className='text-[9px] uppercase text-zinc-500 mt-0.5'>
                      {activeCard.category === 'duitnow'
                        ? 'DuitNow ID'
                        : `Format: ${activeCard.format.toUpperCase()}`}
                    </p>
                  </div>
                </div>
                ...
                {activeCard.notes && (
                  <div
                    ...
                    <h4 className='text-[9px] font-bold uppercase tracking-wider text-zinc-500 mb-0.5'>
                      Notes
                    </h4>
          ...
          <div
            id='hidden-gallery-scanner-modal'
            className='hidden'
            style={{ display: 'none' }}
          />
        </div>
